...
export default class Inky{
    ...
    is_active = false
    blinky
    constructor(field, player, blinky) {
        this.blinky = blinky
        ...
        this.div.style.top = `${this.start_y * this.cell_size - this.img_offset + this.cell_y}px`
    ...
    chase(){
        if(this.cell_x === 0 && this.cell_y === 0){
            ...
            for(let i = 0; i < vars.length; i++){
                let x = this.blinky.x + (this.player.x + this.player.direction[0]*2 - this.blinky.x)*2
                let y = this.blinky.y + (this.player.y + this.player.direction[1]*2 - this.blinky.y)*2
                let cur_len = Math.sqrt(Math.pow(vars[i][0] - x, 2)
                    + Math.pow(vars[i][1] - y, 2))
                if(cur_len < min_len){
                    min_len = cur_len
                    dir = dirs[i]
                }
